...
exports.getPackagingStoreBaleStock = getPackagingStoreBaleStock;
...
async function getPackagingStoreBaleStock() {
    await (0, store_seed_service_1.ensureDefaultStores)();
    const { sourceLocationId, destLocationId } = await getStoreLocationIds();
    // 1. Sum bales produced per inventory item across all packaging runs.
    const outputs = await server_1.prisma.packagingRunFinishedProductOutput.findMany({
    ...
    for (const o of outputs) {
        if (!o.inventoryItemId || !o.inventoryItem)
            continue;
    ...
    const transferredItems = await server_1.prisma.stockTransferItem.findMany({
        where: {
            transfer: {
                sourceLocationId,
                destinationLocationId: destLocationId,
...
async function getBaleTransferById(auth, transferId) {
    ...
    return server_1.prisma.$transaction(async (tx) => {
        ...
        const outputByItemId = new Map(runOutputs.map((o) => [o.inventoryItemId, o]));
