...
exports.PROCUREMENT_REPORT_TYPES = void 0;
exports.generateProcurementReportBuffer = generateProcurementReportBuffer;
const exceljs_1 = __importDefault(require("exceljs"));
const server_1 = require("../server");
exports.PROCUREMENT_REPORT_TYPES = [
    {
...
async function buildWorkbook(reportType, from, to) {
    ...
    if (reportType === "supplier-directory") {
        ...
        styleHeader(sheet);
        const suppliers = await server_1.prisma.supplier.findMany({ orderBy: { name: "asc" } });
    ...
    if (reportType === "requisitions") {
        ...
        styleHeader(sheet);
        const reqs = await server_1.prisma.purchaseRequisition.findMany({
    ...
    if (reportType === "purchase-orders") {
        ...
        styleHeader(sheet);
        const pos = await server_1.prisma.purchaseOrder.findMany({
            where: { createdAt: { gte: fromDate, lte: toDate } },
            orderBy: { createdAt: "desc" },
            include: {
    ...
    if (reportType === "po-line-items") {
        ...
        styleHeader(sheet);
        const lines = await server_1.prisma.purchaseOrderLine.findMany({
    ...
    if (reportType === "spend-by-supplier") {
        ...
        styleHeader(sheet);
        const agg = await server_1.prisma.purchaseOrder.groupBy({
            by: ["supplierId"],
            where: { createdAt: { gte: fromDate, lte: toDate } },
            _count: { _all: true },
        ...
        const suppliers = await server_1.prisma.supplier.findMany({
    ...
    if (reportType === "payment-vouchers") {
        ...
        styleHeader(sheet);
        const vouchers = await server_1.prisma.paymentVoucher.findMany({
            where: { createdAt: { gte: fromDate, lte: toDate } },
            orderBy: { createdAt: "desc" },
            include: {
                threeWayMatch: {
...
async function generateProcurementReportBuffer(reportType, from, to) {
