...
async function listInvoicesController(req, res) {
    const parse = invoice_schemas_1.ListInvoicesQuerySchema.safeParse(req.query);
    if (!parse.success) {
    ...
    const invoices = await (0, invoice_service_1.listInvoices)(parse.data);
    res.status(200).json({ invoices });
}
async function getInvoiceController(req, res) {
    const invoice = await (0, invoice_service_1.getInvoiceById)(req.params.id);
