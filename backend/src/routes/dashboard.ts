...
dashboardRouter.get("/summary", async (_req, res) => {
    try {
        ...
        const rawMaizeAgg = await prisma.rawMaizeBatch.aggregate({
            where: { status: "APPROVED" },
            _sum: { currentQuantity: true },
        });

        // Finished goods stock by grade
        const grade1 = await prisma.finishedGoodsBatch.aggregate({
            where: { productType: "GRADE_1_FLOUR" },
            _sum: { currentQuantity: true },
        });
        const grade2 = await prisma.finishedGoodsBatch.aggregate({
            where: { productType: "GRADE_2_FLOUR" },
            _sum: { currentQuantity: true },
        });

        // Production yield history: last 6 runs by startTime
        const yieldHistory = await prisma.productionRun.findMany({
        ...
        const avg = await prisma.productionRun.aggregate({
        ...
        const avgYieldLossRate = await prisma.productionRun.aggregate({
        ...
        const payload = {
            ...
            recentActivities: await prisma.productionRun.findMany({
                orderBy: { startTime: "desc" },
                take: 4,
                select: {
                    runNumber: true,
