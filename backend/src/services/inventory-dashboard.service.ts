...
export async function getInventoryDashboardAnalytics(storeCode?: string) {
  ...
  if (locationId) {
    ...
    const topStockByValue: Array<{ sku: string; name: string; quantity: number; unit: string; value: number }> = [];
    const belowReorder: Array<{ id: string; sku: string; name: string; quantity: number; reorderLevel: number; unit: string; shortfall: number }> = [];
