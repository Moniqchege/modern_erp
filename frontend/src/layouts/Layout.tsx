...
export function Layout({ children, activeTab, setActiveTab }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex antialiased selection:bg-indigo-650 selection:text-white relative">
      ...
      {/* Sidebar Navigation - Clean High Contrast Light Mode */}
      <aside className="w-72 bg-white border-r border-slate-200/80 flex flex-col justify-between shrink-0 relative z-20 shadow-sm">
        <div>
          {/* Logo Brand Header */}
              ...
              return (
                ...
                >
                  ...
                  <div className={`p-1.5 rounded-lg transition-colors ${
                    isActive ? "bg-indigo-100 text-indigo-650" : "bg-slate-100 text-slate-400 group-hover:text-slate-650 group-hover:bg-indigo-50/50"
                  }`}>
        ...
        <div className="p-4 border-t border-slate-200/80 bg-slate-50/50">
          <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
            ...
            <button className="text-slate-400 hover:text-rose-600 p-1.5 hover:bg-rose-50 rounded-lg transition-colors" title="Log Out">
        ...
        <header className="h-16 border-b border-slate-200/80 bg-white/80 backdrop-blur-md px-8 flex items-center justify-between shrink-0 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] text-slate-400 hidden md:block font-medium">
              Press <kbd className="bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded text-[9px] text-slate-500">Ctrl + K</kbd>
            </span>
          </div>
